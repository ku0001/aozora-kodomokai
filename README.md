...
以下1箇所がまだ `◯◯` のプレースホルダーのままです。値が決まり次第、該当ファイルを
「TODO(要確認)」で検索して書き換えてください。

- **参加費用**: `index.html` の「活動要項」セクション（"◯◯円"）

### 2. 学生メンバー紹介・保護者の声

`members.html` と `index.html`（保護者・子どもの声セクション）は、
実データが届くまで「準備中」表示になっています。

本人の許可を得た写真・プロフィール・感想が揃ったら、それぞれのファイル内にある
`<!-- コンテンツ差込予定 -->` コメントの直後にあるテンプレート（コメントアウト済み）
を使って差し替えてください。手順は各ファイル内のコメントに記載しています。
